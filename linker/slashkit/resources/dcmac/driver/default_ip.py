...
def decode_control_register(value: int) -> dict:
    return {
        'ap_start':     bool(rshift(value)),
        'ap_done':      bool(rshift(value, 1)),
        'ap_idle':      bool(rshift(value, 2)),
        'ap_ready':     bool(rshift(value, 3)),
        'ap_continue':  bool(rshift(value, 4)),
        'auto_restart': bool(rshift(value, 7)),
    }
...
cregs = {
    "controlreg": {'offset': 0x0, 'type': 'rw',
                   'fields': {'ap_start': {'start': 0, 'length': 1, 'default': 0, 'type': 'rw'},
                              'ap_done': {'start': 1, 'length': 1, 'default': 0, 'type': 'rw1c'},
    ...
    "intstatus": {'offset': 0x0C, 'type': 'rw'},
}


class DefaultIP(GenericMMIO):
    """Generic IP Driver"""
    _controlreg   = cregs["controlreg"]["offset"]
    _globalintreg = cregs["globalintreg"]["offset"]
    _intenable    = cregs["intenable"]["offset"]
    _intstatus    = cregs["intstatus"]["offset"]
